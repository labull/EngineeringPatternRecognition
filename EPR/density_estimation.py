...
class student_t:
    ...
    def logpdf(self, X):
        ...
        lp = (gammaln(self.v / 2 + self.D / 2) - gammaln(self.v / 2) -
              ...
              (self.v + self.D) / 2 * np.log(
                    1 + 1 / self.v * np.sum(Lu.T * Lu.T, 1)))
        return lp
...
class NIW:
    ...
    def train(self, X):
        ...
        # init post. predictive
        S = (self.kn + 1) / (self.kn * (self.vn - self.D + 1)) * self.Sn
        self.post_pred = student_t(self.mn, S, self.vn - self.D + 1)
    ...
    def predict(self, Xt):
        # (update) posterior-predictive given data Xt
        m = self.mn
        S = (self.kn + 1) / (self.kn * (self.vn - self.D + 1)) * self.Sn
        nu = self.vn - self.D + 1
