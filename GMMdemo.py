...
plt.figure(1, figsize=[5,6], dpi=300)
...
c = plt.contourf(xx, yy, np.exp(LP), alpha=.9, levels=50, cmap='GnBu_r')
plt.scatter(X[:,0], X[:,1], s=1, alpha=.5)
plt.colorbar(c)
...
plt.figure(4, figsize=[5,6], dpi=300)
for k  in range(K):
    e = ellipse(GMM.base[k].mu_map, GMM.base[k].Sig_map).cov_3
...
# plot lml
plt.figure(5, figsize=[5,6], dpi=300)
plt.plot(GMM.lml)
...
plt.figure(6, figsize=[5,6], dpi=300)
# ellipses
for k  in range(K):
    e = ellipse(GMM.base[k].mu_map, GMM.base[k].Sig_map).cov_3
    plt.plot(e[2][:,0], e[2][:,1], 'r', lw=.8) # map cluster
...
# plot lml
plt.figure(7, figsize=[5,6], dpi=300)
plt.plot(GMM.ll)

#%% --- uncertainty sampling demo (for active learning)
...
plt.figure(8, figsize=[4,6], dpi=300)
# ellipses
for k  in range(K):
    e = ellipse(GMM.base[k].mu_map, GMM.base[k].Sig_map).cov_3
...
plt.scatter(Xu[li,0], Xu[li,1], marker='o', 
            s=5e3*np.exp(GMM.lpx_ul[li]), c='g', alpha=.3)
